...
void main(){
    ...
    float noise = Noise3d(normalize(star_pos));

    //We read the tint texture according to the position of the sun and the weather factor
    vec2 tintTexCoords = vec2((sun_norm.y + 1.0) / 2.0, max(0.01,pos_norm.y));
    tintTexCoords.y = clamp(tintTexCoords.y + noise * 0.02 - 0.01, 0.01, 0.99);

    vec3 color_wo_sun = texture(tint2, tintTexCoords).rgb;
    vec3 color_w_sun = texture(tint, tintTexCoords).rgb;
    ...
    if(radius < 0.05){//We are in the area of the sky which is covered by the sun
        float time1 = clamp(sun_norm.y,0.01,1);
        radius = radius/0.05;
        if(radius < 1.0-0.001){//< we need a small bias to avoid flickering on the border of the texture
            ...
            vec4 sun_color = texture(sun,vec2(radius,1.0 - time1));
    ...
    //Mist
    float mistStrength = clamp( - pos_norm.y * 5.0, 0.0, 1.0);
    float fullMist = length(color) / 1.7;
    vec3 fullMistColor = vec3(fullMist, fullMist, fullMist);
    color = mix(color, fullMistColor, mistStrength);
